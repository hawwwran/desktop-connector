...
class VaultDisconnectSourceTests(unittest.TestCase):
    ...
    def test_wizard_creates_vault_on_real_relay(self) -> None:
        ...
        # Post-Phase-H: only the sharded create-vault payload remains.
        # The legacy ``initial_manifest_ciphertext`` field was removed
        # alongside the rest of the unified-manifest wire surface.
        self.assertIn(
            '"initial_root_ciphertext": base64.b64encode(initial_root_ciphertext).decode("ascii")',
            runtime_source,
        )
        self.assertNotIn("initial_manifest_ciphertext", runtime_source)
