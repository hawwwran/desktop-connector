...
**Canonical reference**: [`docs/vault-architecture.md`](docs/vault-architecture.md) is the single overview doc — read it first when discussing Vault. The bullets below are a quick pointer map; depth lives there.
...
- **Manifest shape**: the encrypted manifest is **sharded** — a small root envelope (vault-wide metadata + folder pointers, `dc-vault-root-v1` schema) plus one per-folder shard envelope (file entries, `dc-vault-shard-v1` schema) per remote folder. Most edits use `PUT /api/vaults/{id}/folders/{folder_id}/shard-with-root` to atomically advance one shard + the root; folder set / vault-wide policy edits publish the root alone. Spec: `docs/protocol/vault-v1.md` §6.4–§6.8, formats §10. Scoping: `docs/plans/vault-manifest-sharding.md`. The pre-sharding single-envelope shape never shipped.
- **Server tree**: `server/src/Controllers/Vault{,Grants}Controller.php`, `server/src/Repositories/Vault*.php`, `server/src/Crypto/VaultCrypto.php`, `server/src/Auth/VaultAuthService.php`, `server/src/Http/VaultApiError.php`, `server/migrations/002-005_vault*.sql`.
- **Desktop tree**: `desktop/src/vault*.py` covers crypto (Argon2id, AEAD, HKDF labels in `vault_crypto.py`), passphrase wordlist, manifests, sync engine (`vault_binding_*.py`, `vault_filesystem_watcher.py`, `vault_runtime_watchers.py`), data path (`vault_upload.py`, `vault_download.py`, `vault_eviction.py`, `vault_restore.py`, `vault_clear.py`), grants + QR join (`vault_grant*.py`), export/import (`vault_export.py`, `vault_import*.py`), migration (`vault_migration*.py`), diagnostics (`vault_logging.py`, `vault_debug_bundle.py`, `vault_integrity.py`, `vault_repair.py`). The encrypted op-log (Activity tab feed) splits into producer (`vault/state/op_log.py` — `build_op_log_entry` + `append_op_log_entries` with `MAX_OP_LOG_TAIL=200` drop-oldest) and consumer (`vault/state/activity.py` — `normalize_op_log_entry` + `merge_timeline` + `filter_timeline`); see [`docs/plans/activity-timeline.md`](docs/plans/activity-timeline.md).
- **GTK windows**: each runs as a separate subprocess, spawned via `python3 -m src.windows vault-*`. Settings + onboard live under `windows_vault/` (main_window, onboard_window, tab_devices, tab_recovery, tab_danger, tab_migration, tab_activity, tab_maintenance, grant_device_dialog, fresh_unlock_prompt). The single-purpose wizards are top-level modules: `windows_vault_browser/` (file/folder browser), `windows_vault_import.py` (import bundle with per-folder conflict resolution — §5.H2), `windows_vault_export.py` (write + verify + shred — §6.H3), `windows_vault_join.py` (QR-grant claimant — §5.C2), `windows_vault_migration.py` (relay migration wizard — §5.C1), `windows_vault_rotate.py` (access-secret rotation with crash-recovery marker — §5.H3). The Folders tab inside Vault Settings owns the bindings UI (`vault_folders_tab.py`).
...
- **Config knobs** (`server/data/config.json`): `vaultQuotaBytes` (default `1073741824` / 1 GB) stamps the per-vault ciphertext cap onto freshly-created vaults via `VaultsRepository::create`; existing rows aren't retroactively resized. `vaultAuthLimit` (default `120`, **floor `10`**) is the per-(device, vault) auth attempts-per-minute cap. Every authenticated `/api/vaults/{id}/*` call bills one attempt (successful or failed) so a single user session — Vault Settings open + vault browser open + a sync cycle for a few bindings — easily exceeds the original hardcoded 10; the default was raised to 120 on 2026-05-20 to absorb realistic single-user workloads. `Config::vaultAuthLimit()` clamps below-floor configs back to 10 so a typo can't *weaken* the throttle. Operators on busy hosts can raise further.
...
python3 -m src.windows {vault-onboard|vault-main|vault-browser|vault-import|vault-export|vault-join|vault-migration|vault-rotate|vault-passphrase-generator} --config-dir=~/.config/desktop-connector [--vault-id=ABCD-2345-WXYZ]
...
- Pong uses 5 s-timeout OkHttp, synchronous on `onMessageReceived` thread (inside FCM's ~10 s wakelock) — guaranteed completion without detached workers. (Was 3 s; android_logs_8 caught a 22-min window of duration_ms=~3100 timeouts driving a retry storm.)
