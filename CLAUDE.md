...
- **Config knobs** (`server/data/config.json`): `vaultQuotaBytes` (default `1073741824` / 1 GB) stamps the per-vault ciphertext cap onto freshly-created vaults via `VaultsRepository::create`; existing rows aren't retroactively resized. `vaultAuthLimit` (default `10`, **floor `10`**) raises the per-(device, vault) auth attempts-per-minute cap above the original §1.H1 hardcoded value; `Config::vaultAuthLimit()` clamps below-floor configs back to 10 so a typo can't *weaken* the throttle. Both knobs surfaced from the B5 live test (2026-05-19); sync workloads that bill an auth attempt per chunk PUT now have a documented escape hatch.
