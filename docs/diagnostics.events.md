...
| `vault.eviction.alarm_purged_oldest` | desktop | info | `event`, `freed_bytes`, `freed_chunks`, `paths` | ADR 2026-05-18 — destructive deletion inside an alarm-approved (passphrase-gated) cleanup after `used > quota` was detected. Separate from `auto_purged_oldest` so audit logs distinguish "tamper / quota-shrink response" from "fit-an-upload retry" |
| `vault.eviction.alarm_used_exceeds_quota` | desktop | warning | `used`, `quota` | ADR 2026-05-18 — relay's 507 reported `used_bytes > quota_bytes`. Under normal operation the server denies overflow at init, so this is the unambiguous quota-shrink / tamper signal. Uploads suspended until the passphrase-gated alarm cleanup runs |
| `vault.eviction.auto_purged_oldest` | desktop | info | `event`, `freed_bytes`, `freed_chunks`, `paths` | ADR 2026-05-18 — destructive deletion under transparent auto-purge (the upload pauses while the oldest unexpired tombstone or non-current version is dropped to fit the failing upload). One event per candidate the loop drains |
...
| `vault.eviction.orphan_reap_noop` | desktop | warning | `vault`, `candidates` | §4.M1 — orphan reaper found N server-side candidates the manifest doesn't reference, but the server contested every one as still-referenced (concurrent publish reclaimed them between list_chunks and gc_plan). N1 review: surfaced at warning level so persistent noop-loops are visible — operators chasing "why isn't this leak being reaped" can see the candidate count + retry interval (3600s default) without DEBUG logging |
...
| `vault.rotate.marker_clear_failed` | desktop | warning | _exception_ | B1 (post-§5.H3 review) — keyring save succeeded but the in-progress marker couldn't be deleted; non-fatal because the next wizard launch's recovery probe sees the new secret already works and discards the marker |
| `vault.rotate.marker_clear_failed_post_error` | desktop | warning | _exception_ | B1 — rotation aborted before server commit AND clearing the stale marker also failed. Marker stays but probe-on-relaunch will discover the old secret still works and discard |
| `vault.rotate.marker_read_exception` | desktop | warning | `vault` (truncated) | B1 — error reading the rotation marker file at wizard launch. Wizard falls through to the normal confirm flow |
| `vault.rotate.marker_read_failed` | desktop | warning | `path`, `error` | B1 — marker file existed but couldn't be parsed (truncated JSON, corruption); treated as absent so the wizard runs normally |
| `vault.rotate.marker_recovered` | desktop | info | `vault` (truncated) | B1 — crash-recovery probe found the relay holding the new secret (cached secret returns 401), saved marker's new_secret into the keyring, and cleared the marker. Vault is no longer bricked |
| `vault.rotate.probe_unexpected` | desktop | warning | `status` | B1 — crash-recovery probe got an HTTP status that's neither 200 nor 401 (e.g. 5xx, 404). Treated as network_error; marker preserved for retry |
| `vault.rotate.probe_unreachable` | desktop | warning | `error` | B1 — crash-recovery probe couldn't reach the relay at all. Marker preserved; user can retry |
