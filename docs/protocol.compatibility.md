...
| `/api/devices/register` with already-known `public_key` | Was 200 + existing `auth_token`; now **409 Conflict** with no `auth_token` field | technically breaking, but fixed clients short-circuit on persisted creds and never hit this path. Closes the credential-leak vector documented in `temp/finished-plans/desktop-multi-device-support.md` D9 (M.11). Loss of the local auth_token now requires keypair rotation, which produces a fresh device_id and a fresh row. |
...
| Fasttrack `fn=find-phone` | Receivers (desktop M.8+) also accept `fn=find-device` as an alias | extending — senders stay on `find-phone` until both platforms migrate per D5 of `temp/finished-plans/desktop-multi-device-support.md` |
