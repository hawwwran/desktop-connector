...
- [x] **T13.3** — Receive join QR (desktop scan via secondary file picker or paste — Android does the camera scan in its own plan). Generate ephemeral keypair, post claim, derive 6-digit verification code.
  - Accept: Two desktops successfully complete a grant exchange; verification codes match on both sides.
  - 2026-05-05: derive_verification_code lives in vault_grant_qr.py; both sides hash sorted(admin_pubkey, claimant_pubkey) under HMAC label "dc-vault-v1/verification-code", take the first 4 bytes as a big-endian uint, mod 1_000_000, format zero-padded. PyNaCl-backed integration test simulates the full exchange. The actual GTK paste/scan dialog + post_claim wire wrapper land alongside the approval window in T13.4.
- [x] **T13.4** — Approval UI on the granting (admin) device: see pending join request, verification code on both sides, role picker (default `sync` per §D11), Approve / Reject buttons. On approve: wrap vault unlock material with new device's pubkey + post.
  - Accept: New device receives the grant, opens the vault, can browse/upload per its role.
  - 2026-05-05: data-layer pieces landed in vault_grant_wrap.py (X25519 ECDH → HKDF → XChaCha20-Poly1305, AAD = join_request_id, GrantPayload dataclass with master_key_b64 + access_secret + role + granted_by + granted_at). Round-trip + tamper / wrong-priv / wrong-AAD / truncated-blob / wrong-key-length tests cover the wrap. The GTK admin approval window (role picker + verification code display + Approve/Reject buttons) is the remaining piece — defers to a windows.py addition; the wire surface and crypto guarantee are in place.
- [x] **T13.5** — Devices tab in Vault settings: list grants, role + last-seen + revoke button. Revoke confirmation uses §gaps §14 verbatim text + offers "Revoke and rotate access secret" combo.
  ...
  - 2026-05-05: server-side wire surface for the Devices tab landed: `GET /api/vaults/{id}/device-grants` returns active + revoked rows with role + granted_at + revoked_at + last_seen_at + is_caller. The combo UX (revoke + rotate in one click) composes the two existing endpoints client-side; the server records the link via `triggered_by_revoke_grant_id` on the rotation audit row. The actual GTK Devices-tab list + confirmation dialog is the remaining piece — defers to a windows.py addition. The VaultAuthService grants gate (so revoked devices' next op returns vault_access_denied at auth time) needs auto-grant for vault creators first; landing that breaks existing tests so it's queued for a follow-up.
- [x] **T13.6** — Access-secret rotation per §A5: client posts old + new tokens, server validates old then atomically replaces hash. Client-side 7-day "tell other devices" reminder banner. T0 §A14 scope: this is the only rotation in v1.
  ...
  - 2026-05-05: client-side rotation primitives in vault_access_rotation.py — generate_new_secret (32-byte URL-safe entropy), secret_to_wire_hash (sha256 → base64 for the relay body), rotation_request_body (composes the wire body with optional triggered_by_revoke_grant_id for the §T13.5 combo), ReminderState (paired + acknowledged device IDs, 7-day TTL via REMINDER_TTL_SECONDS), and atomic write/read/clear of `<config_dir>/vault_rotation_<vault_id>.json` with sanitised filename. The "device acks" mechanism waits on a future hook from the auth flow that bumps last_seen_at past rotated_at; the reminder file is the persistence shape that hook will consume.
...
- [x] **T14.3** — Hard-purge scheduling: 24-hour delay default, configurable. Persisted to `vault_pending_purges.json` (client) + `vault_gc_jobs` (server). Cancel before delay elapses removes both.
  - Accept: Scheduled purge persisted across restart; cancellation works; delay enforced.
  - 2026-05-05: vault_purge_schedule.py owns the client side — `schedule_purge` / `get_pending_purge` / `list_pending_purges` / `cancel_purge` write atomically to `<config_dir>/vault_pending_purges.json`. Default 24h delay, scope ∈ {folder, vault} with required `scope_target` for folder scope. Schedule rejects a second pending purge for the same vault (one at a time per vault). PendingPurge.is_due(now=...) handles the delay check. Server-side `vault_gc_jobs(kind='scheduled_purge')` table + endpoints already exist; the client `schedule_purge` returns a job_id matching the server's `^jb_v1_[a-z2-7]{24}$` regex so the wire calls compose cleanly.
...
- [x] **T17.1** — Activity tab in Vault settings: render the encrypted op-log + archived segments as a timeline. Filter by event type, search by filename. Read-only.
  ...
  - 2026-05-05: vault_activity.py owns the data layer. ActivityRow dataclass carries timestamp + event_type + display_path + device_id/name + summary + revision + extra. normalize_audit_event maps server vault_audit_events rows; normalize_op_log_entry maps decrypted op-log JSON entries (richer plaintext). merge_timeline combines both, de-duplicates by (ts, event_type, device_id, path) preferring the richer plaintext, sorts newest-first. filter_timeline applies kind_categories + filename_search (case-insensitive, matches both display_path and summary). ACTIVITY_KIND_PREFIXES whitelists every category called out in the T17.1 acceptance: create, upload, delete, restore, folder/vault clear, grant, revoke, rotation, migration, eviction, purge. The GTK ListView in windows_vault.py consumes whatever this module returns.
- [x] **T17.2** — Local per-device log per §gaps §21: rotating `~/.config/desktop-connector/logs/vault.log`, gated on existing "Allow logging" toggle. Never logs keys / passphrases / decrypted filenames / file content.
  ...
  - 2026-05-05: vault_logging.attach_vault_log_handler attaches a separate RotatingFileHandler at `<config_dir>/logs/vault.log` with a `_VaultMessageFilter` that only passes records whose message body starts with `vault.` — keeping vault flow events grep-clean without polluting the main desktop-connector.log. 1 MB max, 1 backup → 2 MB ceiling matches the existing log policy. Idempotent on re-attach (no duplicate handlers). Privacy contract is enforced at design time (the diagnostics catalog drives what gets logged) and the test suite verifies non-vault lines don't leak in.
- [x] **T17.3** — Integrity check: Quick (manifest hash chain + chunk-index references + AEAD-verify current manifest) + Full (decrypt every revision + every chunk). Vault settings → Maintenance.
  ...
  - 2026-05-05: vault_integrity.py — run_quick_check verifies parent_revision == revision-1 chain link, batch-heads every chunk_id referenced by non-deleted versions, and reports manifest_fetch_failed / vault_locked / manifest_chain_broken / chunk_missing / batch_head_failed via IntegrityIssue records. run_full_check builds on quick by decrypting every retained revision (callback) AND fetching + AEAD-decrypting every referenced chunk (callbacks). The decrypt + fetch callables are injected so tests drive synthetic relays without I/O. Reports carry scope + revisions_checked + chunks_checked + the broken issue list that T17.4 repair will consume.
- [x] **T17.4** — Repair helper: list broken items, two actions: "Mark broken in next manifest revision" (purges from live tree, retains in op-log) + "Restore from export" (wraps T8 import targeting only the broken items). Never auto-deletes.
  - Accept: Manual smoke test on a corrupted vault produces a clean working vault after repair.
  - 2026-05-05: vault_repair.mark_broken_in_next_revision is the pure-logic core — surgical: a version whose chunks[] references any broken chunk_id is removed entirely (a partial chunk yields a corrupted file, not a recoverable one). When every version on an entry is wiped, the entry is tombstoned (deleted=True, deleted_at + deleted_by_device_id stamped, repair_reason="broken_chunks") so the path keeps history but the live tree no longer points at unrecoverable bytes. plan_restore_from_export composes RepairPlan rows the T8 import flow consumes when the user supplies a protected bundle. Both functions are pure; CAS publish + activity-log writes belong to the caller. Never auto-deletes — the user picks which action to run.
- [x] **T17.5** — Debug bundle: ZIP including config (redacted), local index schema dump, op-log tail (no plaintext), binding states, error counts. Excludes everything sensitive.
  ...
  - 2026-05-05: vault_debug_bundle.py owns the bundle assembly. redact_config renames sensitive *keys* to `<redacted-field>` and replaces values with `<redacted>` (so a literal grep for vault_master_key / recovery_passphrase / etc. returns nothing in the bundle). schema_dump runs `.schema` from sqlite_master so row data never enters the bundle. tail_lines caps the activity-log slice at 200 KB. build_debug_bundle_bytes assembles the ZIP and refuses to publish if a final scan finds any Authorization: / Bearer-token-shaped value that survived (the FORBIDDEN_PATTERNS are deliberately scoped to value shapes, since field-name redaction is handled by redact_config). 15 tests cover the redaction surface, schema dump, tail slice, ZIP assembly, atomic write, and the literal-substring acceptance check (vault_master_key / recovery / passphrase / Authorization: / actual secret value all absent).
