...
**Status:** logged in `temp/finished-plans/review-doubts.md` §4.M1 — desktop can't enumerate vault chunks (no API); the reaper requires either a new `GET /chunks` endpoint or a server-side `KIND_RECLAIM_ORPHAN_CHUNKS` GC job. The leak is bounded by the existing 30-day chunk retention policy server-side.
...
**Status:** logged in `temp/finished-plans/review-doubts.md` §5.C1 — needs explicit scoping; never build a new feature autonomously.
...
**Status:** logged in `temp/finished-plans/review-doubts.md` §5.C2 — memory note `project_vault_multi_device_story.md` already records that v1 ships recovery-kit-only.
...
**Status:** logged in `temp/finished-plans/review-doubts.md` §5.H2 — library is ready (`find_conflict_batches`), but a new wizard page between Preview and Progress with N pickers + "Apply to remaining" buttons is ~300 LOC of new UI. Wizard defaults to `rename` (conservative, no data loss), so shipping behaviour stays safe.
...
**Status:** logged in `temp/finished-plans/review-doubts.md` §5.H3 — library waits for callers; until rotation is wired, nothing breaks. Pre-emptive risk that requires a new tab UI + server endpoint + kit-regeneration prompt to address.
...
**Approach:** New `_wipe_passphrase` helper pops the dict slot + clears the visible entry buffer; invoked on every terminal path (cancel/cancelled/fail/succeed). Python `str` immutability rules out true zeroing; this drops the explicit reference the moment it's no longer needed.
...
**Status:** logged in `temp/finished-plans/review-doubts.md` §5.M2 — depends on §5.C1 migration wizard; bundle the fix with the wizard build.
...
**Status:** logged in `temp/finished-plans/review-doubts.md` §5.M3 — current per-subprocess scope is MORE secure than the spec implies; closing the gap requires either disk-persisting the timestamp (new attack surface) or amending the spec to document the per-subprocess invariant.
...
**Fix landed:** 388f85f 2026-05-17 (docs clarification, no code change)
...
**Approach:** Documented the cost analysis + format-bump migration path in the function's docstring. AEAD failures are constant-time so no timing-attack concern; chunk records hit on first attempt; v2 format should carry the record-type tag outside the AEAD plaintext.
...
**Approach:** Added `EXPORT_PASSPHRASE_MIN_LEN = 8` floor with typed `vault_export_passphrase_too_short` error. Wizard restricts the picker UI to a higher bar separately; this is the safety net for power users invoking the library API directly.

#### §5.M6 — Migration record `previous_relay_url` overwrite-protection is over-cautious — *conditional on §5.C1*
**Status:** logged in `temp/finished-plans/review-doubts.md` §5.M6 — depends on §5.C1 migration wizard UX.
...
**Status:** logged in `temp/finished-plans/review-doubts.md` §6.H2 — server endpoints ship; desktop needs Devices tab + locked §3.3 wording + fresh-unlock-and-admin double-gate (~500 LOC new GTK + HTTP).
