...
- **Wizard-cancellation rule** (revised 2026-05-03): cancelling the wizard mid-flow **never** changes the Vault-active toggle. The wizard wipes its own partial state (in-memory secrets zeroed; optionally shreds an exported kit per the success-screen toggle) and exits, but the toggle stays where the user put it.
  - **Why the change:** the original locked rule auto-flipped the toggle back to OFF on cancel-without-vault, on the assumption that a user who cancels probably didn't really want the feature. User feedback during M1 testing showed this is paternalistic — a user who deliberately turned the toggle ON has stated intent; canceling a passphrase entry to think about it, or to use the generator, or just because they clicked too soon, is not a signal to reverse that intent. Auto-disabling and hiding the tray submenu was confusing.
  - **What replaces it:** wipe partial state, leave the toggle alone, leave the entry points visible. The user toggles OFF themselves if they actually want to disable Vault.
  - The original §A2 audit closure that locked the auto-flip is superseded by this revision; ``vault_ui_state.wizard_cancel_rule`` always returns ``"no_change"``.
- After a vault exists, toggle OFF / ON behavior is per the table above (hide submenu / stop sync, never destructive).
...
| gaps doc § | Topic | Lock |
|---|---|---|
| §1 | Recovery test prompt at vault creation | **Mandatory** (revised 2026-05-03; was "Recommended, not mandatory" with Skip). The wizard's success screen presents one bundled action — "Export and verify recovery kit…" — that (a) writes the kit file at the user's chosen path and (b) re-runs the recovery flow against that saved file + the typed passphrase via `vault.verify_recovery_kit()` (re-derives `wrap_key` via Argon2id+HKDF, AEAD-decrypts the recovery envelope; Poly1305 verifies kit/passphrase end-to-end). Done is gated on verify success + an explicit confirmation checkbox. The earlier "Test now / Skip" prompt was decorative (both branches did the same thing) and was removed. Status (`OK / Not tested / Failed / Stale`) is still shown in Vault settings → Recovery for ongoing health, but the create-time path now always lands in `OK`. Rationale: the kit is the load-bearing recovery artifact; allowing users to skip verifying it sells false confidence. See `desktop/src/vault.py::verify_recovery_kit` and the wizard in `desktop/src/windows.py::show_vault_onboard`. |
