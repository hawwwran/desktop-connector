...
**Branch:** `tresor-vault`
**Status:** All planned waves + the two originally-deferred polish
items are done as of 2026-05-13. Wave history:

- `d67a348` Wave 1 — header bar + selection-driven action bar.
- `1a51c56` Wave 1.5 — breadcrumb into header title.
- `dd396df` Wave 2 — folder sidebar → navigation-sidebar list box.
- `dfd94b3` Wave 3.1 — fixed-position status icon.
- `bf1d7f0` Wave 3 — file cards + per-row hamburger menus + cleanup.
- `c989969` doc reconciliation (plan-doc header pass 1).
- Wave 2.5 — `Adw.OverlaySplitView` wrapper so the sidebar collapses
  on narrow windows; a header-bar toggle button (`sidebar-show-symbolic`)
  reveals / hides it while collapsed. (Chose `OverlaySplitView` over
  `NavigationSplitView` — the latter is for app-section drill-down;
  ours is a simple sidebar-overlay-content split.)
- Wave 3.5 — per-row Versions menu now scrolls the Details pane to
  the Versions heading via `compute_point` + `vadjustment.set_value`,
  scheduled on `GLib.idle_add` so the heading has a valid allocation
